...
class TKLenderAPI {
    ...
    const DAYS_PAST_DUE_TO_ESTIMATE_FOR_SALE = 40;
...
// 	        $log->write($strlog);
	...
	public function __construct() {
		...
		// подключаем класс работы с нотификациями:
		require_once MODX_BASE_PATH . '/DesignAPI/classes/Notify.php';
...
//				"test" => "тестовое значение",
	...
	public function checkToAddLoan($CustomerAuthToken) {
		...
		$arrListStatuses = [
				'Approved', 'Origination', 'Active', 'PastDue', 'AutoProcessing', 'Restructured', 'Rollover', 'RolloverRequested',
                'Reprocessing',	'RolloverRequested', 'DisbursementInProgress', 'WaitingForApproval', 'WaitingForAgreement', 'Saled',
		];
		$response = true;
	...
	*/
	...
	/**
	 * запрашивает доп.договор кредита определенного типа в API CRM
	 * @param int $loanId
	 * @param string $CustomerAuthToken
	 ...
	 * @return array
	 */
	public function getAdditionalContract(string $CustomerAuthToken = '', int $loanId = 0, string $contractType = '', int $elementId = 0) {
	    
	    // проверка существования данных:
	    if (!$loanId || !$CustomerAuthToken|| !$contractType)	{
	        $response = [
	            'Success' => false,
	            'Message' => "Нет входных данных",
	            'error' => 100,
	    ...
	    switch ($contractType) {
	        case 'prolongation':
	            $url = $this->url . "PublicApi/GetRolloverLoanAgreement?loanId={$loanId}&rolloverId={$elementId}";
	            break;
	            
	        case 'restructuring':
	            $url = $this->url . "PublicApi/GetPreviousRestructureLoanAgreement?loanId={$loanId}&restructureId={$elementId}";
	            break;
	    ...
	    $strlog .= "TKLenderAPI.getAdditionalContract METHOD: $contractType"
            . "\nloanId = " .	$loanId . " url = $url"
            . "\nresponse = " . ((isJSON($res)) ? print_r($response, true) : $res) ;
        $log->write($strlog);
	...
	*/
	public function getContract($credid, $CustomerAuthToken) {
		...
		// непосредственно отправка:
		$res = $this->curl($this->url . 'PublicApi/GetLoanAgreement', $headers, $request, 'GET');
		$response = json_decode($res, true);
...
*/
	...
	public function getInitCRM() {
		...
		if ($this->config) {
			...
			$response['affiliate']['Leadgid']['url'] = ($this->config['Leadgid_url'])? : '';	            // url Leadgid
			$response['affiliate']['Leadgid']['daysExpires'] = ($this->config['Leadgid_daysExpires'])? : 0;	// время жизни куки
...
*/
	...
	*/
    ...
    /**
     * Проверка возможности пролонгации
     */
    public function IsLoanCanBeRolloved(string $CustomerAuthToken, int $loanId, string $date = null, string $comments = '') {
        if (!$loanId)	{
			$response = [
					'Success' => false,
					'Message' => "Нет входных данных",
        ...
        $request = [
            'loanId' => $loanId,
            // 'term' => $term,
            // если нет даты - назначаем минимальную (завтра), для проверки принципиальной возможности продления
            'desiredPaymentDate' => $date ?? date_format(date_create('+1 day'), 'Y-m-d H:i:s.v'),
            'comments' => $comments,
        ];
		...
		// непосредственно отправка:
		$res = $this->curl($this->url . 'PublicApi/IsLoanCanBeFrontRolloved', $headers, $request, "GET");
		$response = json_decode($res, true);
		
		// подключаем логирование:
		$log = new Log('prolong_');
		$strlog .= "TKLenderAPI.prolongation METHOD: IsLoanCanBeFrontRolloved " . 
			"\nrequest=" . print_r($request, true). 
			//"\nres=" . print_r($res, true). 
			//"\nresponse=" . print_r($response, true);
			"\nresponse = " . ((isJSON($res)) ? print_r($response, true) : $res);
	...
	public function notifyShowLater(string $CustomerAuthToken = '', int $notificationId = 0, bool $showLater = false) {
	    ...
	    // подключаем логирование:
	    $log = new Log('notify_');
	    $strlog .= "TKLenderAPI.notifyShowLater METHOD: ShowLater"
	        . "\ntoken = {$CustomerAuthToken}"
	        . "\nurl = " . $url
	        . "\nresponse = " . ((isJSON($res)) ? print_r($response, true) : $res);
	    $log->write($strlog);
...
/**
	...
	/**
	 * Отпраляет email службе обслуживания (офису, или партнеру)
	 *
	 * @param array $data
	 * @param string $recipient
	 * @return array
	 */
	public function sendEmailToMaintenance($data, $recipient)
	{
	    // проверка существования данных:
	    if (!is_array($data)) {
	        $response = [
...
// 	    }
	...
	/**
	 * удаляет дополнительную карту
	 * @param int $cardId
	 * @return boolean[]|string[]|number[]|mixed
	 */
	public function tranzzoDeleteCard(int $cardId = 0) {
	    
	    // проверка существования данных: 516800******4960
	    if (!$cardId || !$_SESSION ['token']) {
	        $response = [
	            'Success' => false,
	            'Message' => "Нет входных данных",
	            'error' => 100,
	    ...
	    // непосредственно отправка:
	    $res = $this->curl($this->url . 'PublicApi/DeleteCard', $headers, $request);
	    $response = json_decode($res, true);
	    
	    // подключаем логирование:
	    $log = new Log('cardReg_');
	    $strlog .= "TKLenderAPI.tranzzoDeleteCard METHOD: DeleteCard"
	        . "\nrequest = " . print_r($request, true)
	        . "\nresponse = " . ((isJSON($res)) ? print_r($response, true) : $res);
	    $log->write($strlog);
