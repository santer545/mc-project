...
$modx->setPlaceholder ( 'hash', '20180622' );	// для хеша скриптов
...
// 				$post ['login'] = str_replace ( "-", "", $post ['login'] );
		...
		if (isset($_POST['dopdogovor']) && (0 < count ( $_POST ['dopdogovor'] ))) { // пришли данные с формы для просмотра доп.соглашения
		    ...
		    $modx->setPlaceholder('refId', $post['refId']);   // куда возвращаться после просмотра
		    ...
		    if (isset($_SESSION ['credit']['Id'])) {
		        ...
		        switch ($post['type']) {
		            ...
		            case 'prolongation':
		                $modx->setPlaceholder('anchorId', 'prolongation-anchor');   // куда возвращаться после просмотра (якорь)
		                $_SESSION['prolongation']['selectedId'] = $post['elementId'];
		                // $res = $api->getAdditionalContract ( $credit_id, $_SESSION ['token'] ); // получить договор
		                $res = $api->getContract ( $credit_id, $_SESSION ['token'] ); // получить договор (для теста)
		                break;
		            case 'restructuring':
		                $modx->setPlaceholder('anchorId', 'restructuring-anchor');   // куда возвращаться после просмотра (якорь)
		                // $_SESSION['prolongation']['selectedId'] = $post['elementId'];
		                // $res = $api->getAdditionalContract ( $credit_id, $_SESSION ['token'] ); // получить договор
		                $res = $api->getContract ( $credit_id, $_SESSION ['token'] ); // получить договор (для теста)
		                break;
		...
		if (isset($_POST['prolongation']) && (0 < count ( $_POST ['prolongation'] ))) { // пришли данные с формы "Пролонгация"
			...
			if (($credit_id = $post ['credit_id']) && (strtotime ( $dateProl ) >= strtotime ( $dateBegin )) && (strtotime ( $dateProl ) <= strtotime ( $dateEnd ))) {
		...
		if (isset($_POST['radiocard']) && (0 < count ( $_POST ['radiocard'] ))) { // пришли данные с формы "Дополнительные карты"
		    ...
		    if ($card) {
		        ...
		        if ($post['action'] === 'main') { 
    		        ...
    		        // $resCard = $api->tranzzoDeleteCard($card);
    		    }
    		    if ($resCard['Success']) {
		            $res = $api->tranzzoGetCardsAll( $_SESSION ['token'] ); // получить список карт клиента
		            if (count($res) > 0)
	...
	case "OnLoadWebDocument" :
		...
		if ($_GET ['utm_source'] || $_GET ['admitad_uid']) {
            ...
            $affiliate = '';
			// узнаем партнера:
			if ($_GET ['utm_source'] == "salesdoubler") {
				$affiliate = 'SalesDoubler';
			} elseif ($_GET ['utm_source'] == "primelead") {
				$affiliate = 'PrimeLead';
			} elseif (isset($_GET ['admitad_uid'])) {
				$affiliate = 'Admitad';
			// } elseif (isset($_GET ['afclick'])) {
			// 	$affiliate = 'Loangate';
			} elseif ($_GET ['utm_source'] == "linkprofit") {
				$affiliate = 'Linkprofit';
			} elseif ($_GET ['utm_source'] == "doaffiliate") {
				$affiliate = 'DoAffiliate';
			} elseif ($_GET ['utm_source'] == "leads.su") {
				$affiliate = 'Leads_su';
			} elseif (substr($_GET['utm_source'], 0, 4) == 'sat_') {
				$affiliate = 'Satellite';
			} elseif ($_GET ['utm_source'] == "targetme") {
			    $affiliate = 'Targetme';
			} elseif ($_GET ['utm_source'] == "finline") {
			    $affiliate = 'Finline';
			} elseif ($_GET ['utm_source'] == "moneyexpert") {
			    $affiliate = 'Moneyexpert';
			} elseif ($_GET ['utm_source'] == "uacreditbiz") {
			    $affiliate = 'Uacreditbiz';
			} elseif ($_GET ['utm_source'] == "kadam") {
			    $affiliate = 'Kadam';
			} elseif ($_GET ['utm_source'] == "Letmeads") {
			    $affiliate = 'Letmeads';
			}
			Affiliate::deleteCookiesExcept($affiliate, $https);
		...
		if (isset ( $_SESSION ['token'] )) {
			...
			if ($res ['Success']) {
				...
				if(date( 'dm', strtotime( $res ['Data'] ['BirthDate'])) === date('dm')) {
				    ...
				    $modx->documentObject ['notifyCalculatorType'] = 'isBirthDate';
				}
				$client = $res['Data'];
			} else {
				$client = [];
			...
			if ($res ['Success']) {
				if (isset ( $_SESSION ['res'] ['Data'] )) {
					...
					foreach ($bonuses as $key => $value) {
						...
						// если тип нотификации еще по умолчанию, и есть бонус:
						if (($modx->documentObject ['notifyCalculatorType'] === 'standart') && (substr($value, 0, 11) === 'statusBonus')) {
						    $modx->documentObject ['notifyCalculatorType'] = 'bonus';
						}
					}
				}
			} else {
			...
			case 427 :
				...
				if (! $res ['Success']) {
					...
					if (isset ( $key_id )) {
						...
						// $credit['DaysPastDue'] = 5;							// Для теста !!!
						...
						$modx->documentObject ['daysTermStr'] = getDayLang ( $credit ['Term'], $modx->config ['lang'] );
...
// Для теста:
//$modx->documentObject ['credit_status'] = 'Active';
//$modx->documentObject ['IsLoanCanBeRollovered'] = '1';
//$modx->documentObject ['termEndReal'] = 1;

						$prolongations = [];
						
						if (in_array($credit['Status'], ['Active', 'PastDue',])) {
			...
			case 716 :   // АВ - тест
			    // для теста, удалить:
			    // $modx->documentObject ['isRepeated'] = "1";
			    // $modx->documentObject ['isCreditNotClosed'] = "0";
				// $modx->documentObject ['auth'] = "1";
			    /*
